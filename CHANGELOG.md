...
- FreshRSS article downloads on ESP32-C3 now spool each one-item HTTPS page to SD before parsing (matching the font-manifest pattern), reuse TLS during the download burst, drop duplicate subscription metadata before caching, and report which article index failed on network or low-memory errors.
...
- FreshRSS sync now releases SD-card list fonts and glyph caches before starting TLS, preventing larger authenticated responses from failing on memory-constrained ESP32-C3 devices after ClientLogin succeeds.
