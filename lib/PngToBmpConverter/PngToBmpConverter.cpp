...
bool PngToBmpConverter::pngFileToBmpStreamInternal(FsFile& pngFile, Print& bmpOut, int targetWidth, int targetHeight,
  ...
  if (oneBit) {
    ...
    if (!atkinson1BitDitherer || !atkinson1BitDitherer->isValid()) {
      LOG_ERR("PNG", "OOM: Atkinson 1-bit dither buffers for width %d", outWidth);
      free(rowBuffer);
      free(ctx.currentRow);
      free(ctx.previousRow);
      return false;
    ...
    if (USE_ATKINSON) {
      ...
      if (!atkinsonDitherer || !atkinsonDitherer->isValid()) {
        LOG_ERR("PNG", "OOM: Atkinson dither buffers for width %d", outWidth);
        free(rowBuffer);
        free(ctx.currentRow);
        free(ctx.previousRow);
        return false;
      ...
      if (!fsDitherer || !fsDitherer->isValid()) {
        LOG_ERR("PNG", "OOM: Floyd-Steinberg dither buffers for width %d", outWidth);
        free(rowBuffer);
        free(ctx.currentRow);
        free(ctx.previousRow);
        return false;
  ...
  if (needsScaling) {
    ...
    if (!rowAccum || !rowCount) {
      LOG_ERR("PNG", "OOM: scaling buffers for width %d", outWidth);
      free(rowBuffer);
      free(ctx.currentRow);
      free(ctx.previousRow);
      return false;
